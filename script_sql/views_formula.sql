...
CREATE OR REPLACE VIEW public.vendedores_ventas_detalles
 ...
 SELECT a.id_vendedor,
	...
	g.identificacion || ' ' || g.nombres || ' ' || g.apellidos as tomador,
        CASE
            WHEN d.tpoliza IS NULL THEN 'NO APLICA'
            ELSE d.tpoliza
        END AS poliza_descripcion,
        CASE
            WHEN c.cobertura IS NULL THEN 'NO APLICA'
            ELSE c.cobertura
        END AS cobertura_descripcion,
	a.nombres || ' ' || a.apellidos as vendedor_data
   FROM t_vendedores a
     JOIN t_ventas b ON a.id_vendedor = b.id_vendedor AND b.estatus_venta = 'A'
